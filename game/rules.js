...
export default ({
  ...
  async function createRules(lobbyId) {
    return await db.query(ruleQuery.string, ruleQuery.data.map(a => {
      a.$lobby_id = lobbyId;
      return a
    }));
  }
  ...
  async function deleteRules(lobbyId) {
    return await db.query(`DELETE FROM rules WHERE lobby_id=$lobby_id`, {
      $lobby_id: lobbyId
    });
  }
